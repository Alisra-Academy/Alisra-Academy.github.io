...
function discusFunc() {
    Components.disLinks.forEach(link => {
        link.addEventListener('click', (e) => {
            ...
            window.open(`https://wa.me/923102783397?text=Hi, I'm Intrested in ${nextElementText} Course And want to Discuss About it`);
            e.preventDefault();
        })
    })
}
...
function registerFunc() {
    ...
    Components.registerLinks.forEach(rlink => {
        rlink.addEventListener('click', (event) => {
            regtl.to(Components.modPop, {scale:1,ease:Expo.easeInOut})
    ...
    Components.modalClose.addEventListener('click', (event) => {
        regtl.to(Components.modPop, {scale:0, ease:Expo.easeInOut})
        tform.reverse();
